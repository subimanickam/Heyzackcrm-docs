...
export default async function CrmDocsPage({ params }: PageProps) {
  const { slug } = await params
  const page = crmSource.getPage(slug)
  ...
  const sectionName = findSectionName(tree, page.url, 'Get started')
    ...
    <div className="flex gap-8">
      <article className="flex-1 min-w-0 max-w-3xl">
        ...
        <div className="prose prose-slate dark:prose-invert max-w-none">
          <MDXContent components={getMDXComponents('/docs/crm')} />
        </div>
...
export async function generateStaticParams() {
  return crmSource.generateParams()
}
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const tree = crmSource.pageTree
  const section = findSectionName(tree, page.url, 'Get started')
